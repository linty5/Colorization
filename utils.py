...
def create_generator(opt):
    ...
    if opt.net_mode == 'uresnet':
        generator = network_uresnet.UResNet444(opt)
    ...
    if opt.finetune_path != "":
        ...
        network_uresnet.weights_init(generator, init_type = opt.init_type, init_gain = opt.init_gain)
...
def create_discriminator(opt):
    # Initialize the network
    discriminator = network_uresnet.PatchDiscriminator70(opt)
    # Init the network
    network_uresnet.weights_init(discriminator, init_type = opt.init_type, init_gain = opt.init_gain)
    print('Discriminators is created!')
    return discriminator
    
def create_perceptualnet():
    # Get the first 15 layers of vgg16, which is conv3_3
    perceptualnet = network_uresnet.PerceptualNet()
    # Pre-trained VGG-16
