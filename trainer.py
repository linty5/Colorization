...
def Trainer(opt):
    ...
    sample_imglist = utils.get_jpgs(os.path.join(opt.in_path_sample))
    ...
    sample_dataset = dataset.Qbayer2RGB_dataset(opt, 'sample', sample_imglist)
    ...
    print('The overall number of sample images:', len(sample_imglist))
    ...
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size = opt.val_batch_size, shuffle = True, num_workers = opt.num_workers, pin_memory = True)
    sample_loader = torch.utils.data.DataLoader(sample_dataset, batch_size = opt.train_batch_size, shuffle = False, num_workers = opt.num_workers, pin_memory = True)
