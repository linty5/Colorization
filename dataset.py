...
class Qbayer2RGB_dataset(Dataset):
    def __init__(self, opt, tag, imglist):
        ...
        if tag == 'sample':
            self.in_filelist = build_file_set(opt.in_path_sample, imglist)
            self.RGBout_filelist = build_file_set(opt.RGBout_path_sample, imglist)
    ...
    def __getitem__(self, index):
        ...
        RGBout_img = cv2.imread(RGBout_path)
